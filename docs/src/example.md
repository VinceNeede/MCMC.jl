...
We now define our `main` function. This will increment the semaphore, create a new `PiMCMC` object and start the run. At the end we release the semaphore so that another task can start.
```julia
